...
#|
Design Notes: On to Worm Plus!

If we assume (define-struct game [worm1 worm2 food])

The following do not change (much):
...
game-main (does not change much)
...
(define SEG-IMG1  (circle (/ SEG-SIZE 2) "solid" "red"))
(define SEG-IMG2  (circle (/ SEG-SIZE 2) "solid" "blue"))
...
;;; interpretation: (make-game w1 w2 f) is a game state where w1 and w2
;;;   are the worms and f is the grid position of the single piece of food.
;;;
...
(define GAME-1 (make-game WORM-1 WORM-3 (make-posn 10 10))
...
(define (game-render g)
  (render-segments (worm-segs (game-worm1 g)) SEG-IMG1
                   (render-segments (worm-segs (game-worm2 g)) SEG-IMG2
                                    (place-image FOOD-IMG
                                                 (grid->px (posn-x (game-food g)))
                                                 (grid->px (posn-y (game-food g)))
                                                 BG)))
