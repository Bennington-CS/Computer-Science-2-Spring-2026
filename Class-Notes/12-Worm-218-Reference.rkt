...
;;;
;;; Has the worm's head moved outside the grid?  Consumes a Worm and
;;; produces #true when the head's x or y coordinate is out of bounds.
;;;
;;; (define (worm-hit-wall? w)
;;;   (... (posn-x (first (worm-segs w)))
;;;    ... (posn-y (first (worm-segs w)))
;;;    ... GRID-W ... GRID-H ...))
;;;
;;; w : a Worm
;;;
...
;;;
;;; Has the worm's head landed on one of its own tail segments?  Consumes
;;; a Worm and produces #true when the head posn appears in the rest of
;;; the segment list.  Uses member? to search the tail.
;;;
...
;;; worm-dead? : Worm -> Boolean
;;;
;;; Should the game stop?  Produces #true when the worm has hit a wall or
;;; run into itself.  Consumes a Worm and produces a Boolean.
;;;
;;; (define (worm-dead? w)
;;;   (or (worm-hit-wall? w) (worm-hit-self? w)))
;;;
;;; w : a Worm
;;;
(check-expect (worm-dead? WORM-3) #false)
(check-expect (worm-dead? (make-worm (list (make-posn -1 5)) "left")) #true)
(check-expect (worm-dead?
                (make-worm (list (make-posn 5 5) (make-posn 6 5) (make-posn 5 5)) "left"))
              #true)
